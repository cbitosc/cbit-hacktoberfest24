...
export default function TeamDetails() {
      ...
      try {
        ...
        if (teamDoc.exists()) {
          ...
          const duplicateQuery = query(
            collection(db, 'teams'),
            where('participants', 'array-contains-any', participantEmails)
          );
          const duplicateSnapshot = await getDocs(duplicateQuery);
          
          if (duplicateSnapshot.size > 1) {
            toast.error("One or more participants are registered in multiple teams. This is not allowed.");
            setTeamData(null);
          ...
          const hasViewedTeamDetails = localStorage.getItem('hasViewedTeamDetails');
  ...
  if (!user) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-darkgrey">
        <div className="text-white text-xl">Please log in to view team details</div>
      </div>
    );
  }
  ...
  if (!teamData) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-darkgrey">
        <div className="text-white text-xl">No team registration found</div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="border-b border-green pb-4">
            ...
            <p className="text-pink mt-2">Registered on: {teamData.createdAt.toDate().toLocaleDateString()}</p>
          </div>
              ...
              {teamData.participants.map((participant, index) => (
                ...
                >
                  <h3 className="text-lg font-semibold text-pink mb-2">
                    {participant.isTeamLeader ? "Team Leader" : `Member ${index + 1}`}
                  </h3>
                  <div className="space-y-2 text-white">
                    <p><span className="text-green">Name:</span> {participant.name}</p>
                    <p><span className="text-green">Email:</span> {participant.email}</p>
                    <p><span className="text-green">Phone:</span> {participant.phone}</p>
                    <p><span className="text-green">Roll No:</span> {participant.rollNo}</p>
                    <p><span className="text-green">Institution:</span> {participant.institution === 'other' ? participant.otherInstitution : participant.institution}</p>
                    <p><span className="text-green">Branch:</span> {participant.branch === 'other' ? participant.otherBranch : participant.branch}</p>
                    <p><span className="text-green">Year:</span> {participant.yearOfStudy}</p>
                    <p><span className="text-green">Section:</span> {participant.section}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {teamData.techStack.map((tech, index) => (
                <span key={index} className="bg-green/20 text-green px-3 py-1 rounded-full text-sm">
                  {tech}
                </span>
              ))}
            </div>
